...
class LearningAgent(Agent):
    ...
    def reset(self, destination=None, testing=False):
        ...
        if testing:
            ...
            #self.epsilon = 1.0 - self.x / 100.0
            #self.epsilon = math.pow(0.91,self.x)
            self.epsilon = math.exp(-0.1*self.x)
            #self.epsilon = 1 / math.pow(self.x,2)
            self.x += 1
            #self.epsilon = (self.x/((math.pow(self.x,2))-1))
    ...
    def choose_action(self, state):
        ...
        elif self.epsilon > 0.01 and random.random() > self.epsilon:
            action = random.choice(self.valid_actions)
        else:
            for a in self.Q[state]:
